...
import time
import data
from desain import save_header
...
def save_data():
    ...
    string_gadget                 = convert_data_to_string_gadget_consumable(data.gadget, data.header_gadget)
    ...
    string_consumable             = convert_data_to_string_gadget_consumable(data.consumable, data.header_consumable)
...
# fungsi mengubah array input menjadi sebuah string
...
def convert_data_to_string_gadget_consumable(file_data, file_header):
# fungsi mengubah array input menjadi sebuah string untuk file gadget dan consumable

    string_data = ";".join(file_header)
    string_data += "\n"
    for arr_data in file_data:
        if (arr_data[3] < 0) : # apabila data telah dihapus, maka diabaikan
            continue
