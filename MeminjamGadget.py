...
from desain import pinjam_header
...
def pinjam():
    ...
    if (not is_id_valid(id)): 
        print(red("Gagal"),"meminjam item karena", red("ID tidak valid.")) 
    elif(not is_id_available(id, datas)):
            print(red("Gagal"),"meminjam item karena", red("ID tidak ada."))
    ...
    if (is_id_valid(id) and is_id_available(id, datas) and is_borrowed == False) :
        ...
        if (not is_tanggal_valid(tgl)) : 
            print("Tanggal peminjaman", red("tidak valid !"))
        else :
            jumlah = int(input("Jumlah peminjaman: "))
            if jumlah > datas[find_raw(id, datas)][3]:
                print(f"Item {datas[find_raw(id, datas)][1]}",red("tidak berhasil dipinjam."),"Jumlah peminjaman terlalu banyak!")
            elif jumlah <= 0 : 
                print(f"Item {datas[find_raw(id, datas)][1]}", red("tidak berhasil dipinjam."),"Jumlah peminjaman harus lebih besar dari 0")
            else :
                print(f"Item {datas[find_raw(id, datas)][1]} (x{jumlah}) telah", green("berhasil"), "dipinjam!")
